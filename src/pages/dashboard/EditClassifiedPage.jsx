...
import { getClassifiedById, updateClassified } from '../../services/classifiedService';
...
export default function EditClassifiedPage() {
  useDocumentTitle('Editar Classificado');
  const { id } = useParams();
  ...
  useEffect(() => {
    getClassifiedById(id)
      ...
      .catch(() => toast.error('Erro ao carregar classificado'))
  ...
  const handleSubmit = async (e) => {
    ...
    setSaving(true);
    try {
      await updateClassified(id, form);
      toast.success('Classificado atualizado!');
  ...
  return (
    <div>
      <h1 className="text-2xl font-bold font-heading text-gray-900 mb-6">Editar Classificado</h1>
      <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
        <Input label="Titulo" id="title" value={form.title} onChange={handleChange('title')} />
        <Textarea label="Descricao" id="description" value={form.description} onChange={handleChange('description')} rows={6} />
        <Select label="Categoria" id="category" value={form.category} onChange={handleChange('category')} options={categoryOptions} placeholder="Selecione" />
        <Input label="URL da imagem" id="imageUrl" value={form.imageUrl} onChange={handleChange('imageUrl')} />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <Input label="Estado" id="state" value={form.state} onChange={handleChange('state')} />
        </div>
